...
if(gettype(session()->get('profiles')) == 'array' ){
if(in_array(13, session()->get('profiles'))){
    $permissions = getPermissionDetails('13','42','35');
		...
		<div class="row">
				...
				<div class="col-md-12">
					<div class="panel panel-info panel-color">
						<div class="panel-heading"><h3 class="panel-title">Admin Time Entry</h3></div>
						<div class="panel-body" style="margin-top:10px">
							<div class="col-md-12">
							<a href="" id="redirect"></a>
							<a href="" id="redirectt"></a>

							<!-- <form class="form-horizontal" role="form"> -->
							<?php 
							$attr = array("class" => "form-horizontal");
							echo form_open("admin/Timesheet/admin_addAttendance", $attr); 							
							?>
							<div class="row">
										...
										<div class="col-md-12" style="padding-bottom: 13px;">
                                           ...
                                           <select id="User_option" name="User_option" class=" form-control select2">
                                                    ...
                                                    <?php foreach ($facultystaff as  $staff) { ?>
                                                     <option value="<?php echo $staff['ID'] ?>" <?php if($empid==$staff['ID']){ echo "selected"; }?> >
										...
										<div class="col-md-12">
											...
											<div class="form-group col-md-12 " style=" margin-left: 1.666667% !important; margin-top: 15px !important;">	
												<div class="form-group m-b-0">
													...
													<?php if($add_permission || $edit_permission){ ?>
											...
											<div class="col-md-12" style="margin-top: 41px; margin-left: 91px;">
											 <?php if($add_permission || $edit_permission){ ?>
											...
											<div class="col-md-12" style="margin-top: 41px;">
													<h4 style="color: #e8330a;">This day's attendance is locked.  
														
														<input type="hidden" name="unlock_empid" id="unlock_empid" value="<?=$empid?>">
												        <button  type = "button" 								        
												          		 class="btn btn-success waves-effect waves-light m-b-3 m-l-3" id="uplock_btn" ><i class="fa fa-unlock-alt fa-lg" aria-hidden="true" ></i>
												        </button>
...
$(document).on('change', "#transaction_date", function(){
	...
	date_val.split('-');
	var empid=<?=$empid?>;
	var base_url = '<?php echo base_url('admin/Timesheet/admin_attendance/');?>';
	var url = base_url+date_val.replace(/\//g , '-')+'/'+empid;
	/*alert(url);*/
...
$(document).on('change', "#User_option", function(){
	...
	var date_val = $("#transaction_date").val();
	date_val.split('-');
	var base_url = '<?php echo base_url('admin/Timesheet/admin_attendance/');?>';
	var url = base_url+date_val.replace(/\//g , '-')+'/'+User_option;
	/*alert(url);*/
	$('#redirectt').attr('href', url);
	$('#redirectt')[0].click();
...
<script >
	...
	var empid=<?=$empid?>;
	var base_url = '<?php echo base_url('admin/Timesheet/admin_attendance/');?>';
	var url = base_url+fdate.replace(/\//g , '-')+'/'+empid;
	/*alert(url);*/
	$('#redirect').attr('href', url);
	$('#redirect')[0].click();
		//console.log(ev);
...
<!-- script for uplock -->
<script>
	$("#uplock_btn").click(function(){
		var empid = $("#unlock_empid").val();
		var date_time_str = $("#transaction_date").val();
		var date_time = date_time_str.split('/');
		var year = date_time[2];
		var month = date_time[0];
		var date = date_time[1];
		var new_date_format = year+'-'+month+'-'+date;
		$.ajax({
			method:'POST',
			url:'<?=base_url('admin/Reports/Update_lock_ajax')?>',
			data:{year:year, month:month, empid:empid, new_date_format:new_date_format},
