				...
				<div class="col-md-12">
					<div class="panel panel-info panel-color">
						...
						<div class="panel-body" style="margin-top:10px">
							...
							echo form_open("admin/Testing/addAttendance", $attr); 							
							?>
							<div class="row">
								...
								<div class="col-lg-9"> 
								       <div class="panel-group panel-group-joined" id="accordion-test"> 
											<?php 
	                                        foreach($pcategory_arr as $pcategory){
	                                            $datas = check_any_sub_category(session()->get('NAME_ID'),$pcategory['id']);
	                                            if(!empty($datas))
	                                            {
								            ...
								            <div class="panel panel-default"> 
								                <div class="panel-heading"> 
								                    <h4 class="panel-title"> 
								                        <a data-toggle="collapse" data-parent="#accordion-test" href="#<?php echo "p_".$pcategory['id'] ?>" class="collapsed" style="color: #337ab7" >
								                           <?php echo $pcategory['catagory_name']; ?>
								                        </a> 
								                    </h4> 
								                </div> 
								                <div id="<?php echo "p_".$pcategory['id'] ?>" class="panel-collapse collapse"> 
								                    <div class="panel-body">
														...
														<div class="col-sm-1">
            										        <input type="checkbox" name="syncData[<?=$count?>][office_status]" value="1" <?php echo $office_status== '1'?'checked':''; ?> <?php if(isset($islock)){ if($islock == 1){ echo "disabled";  } } ?>>
            										   </div>
														<div class="col-sm-2">                                     
															<select name="syncData[<?=$count?>][hours]" <?php if(isset($islock)){ if($islock == 1){ echo "disabled";  } } ?> class="form-control hour">
														...
														<div class="col-sm-2">                                     
															<select name="syncData[<?=$count?>][min]" <?php if(isset($islock)){ if($islock == 1){ echo "disabled";  } } ?> class="form-control minute">
														...
														<div class="col-sm-4">
														  <textarea class="form-control" <?php if(isset($islock)){ if($islock == 1){ echo "disabled";  } } ?> name="syncData[<?=$count?>][journal]" placeholder="Journal" rows="1" ><?php if($journal != '') { echo $journal; }?></textarea>
														</div>
													</div>
								       ...
								       <?php 
                                        ...
                                        if(!empty($category_arr)){
                                        foreach($pscategory_arr as $pscategory){
                                	        $ids[] = "'time".$pscategory['id']."'";
                                	        
                                            $show = true;
                                            $hours = $min = $journal = $office_status = '';
                                            if(isset($group_by_category[$pscategory['id']])){
                                                $show = true;
                                                $hours_with_min = $group_by_category[$pscategory['id']]['hours'];
                                                $journal = $group_by_category[$pscategory['id']]['journal'];
                                                $office_status = $group_by_category[$pscategory['id']]['office_status'];
                                                if($hours_with_min != ''){
                                                    $temp = explode('.', $hours_with_min);
                                                    $hours = $temp[0];
                                                    $min = $temp[1];
								...
								<div class="form-group padding-left-30 grouptime<?=$pscategory['id']?> <?php echo ($show == true ? "show" : "hide"); ?>" style="color: #337ab7;" >
								    

                                                       
								    
										<label for="Hour" class="col-sm-3 control-label"><?=$pscategory['catagory_name']?></label>
										<input type="hidden" name="syncData[<?=$count?>][category_id]" value="<?=$pscategory['id']?>" />
										<div class="col-sm-1">
									        <input type="checkbox" name="syncData[<?=$count?>][office_status]" value="1" <?php echo $office_status== '1'?'checked':''; ?> <?php if(isset($islock)){ if($islock == 1){ echo "disabled";  } } ?>>
									   </div>
										<div class="col-sm-2">                                     
								...
								<?php echo form_close(); }else { echo "No Active Contract"; ?>
		                    		...
		                    		<a href="<?php echo base_url('/admin/Testing/attendance');?>" class="btn btn-success waves-effect waves-light btn-xs m-b-5">
...
$(document).on('change', "#transaction_date", function(){
	...
	date_val.split('-');
	var base_url = '<?php echo base_url('admin/Testing/attendance/');?>';
            ...
            if(confirm("Do you want to save changes?")){
               // $("#timesheet_form").ajaxForm({url: 'Timesheet/addAttendance', type: 'post'})
                ...
                $.ajax({
                    ...
                    url: "<?= base_url() ?>admin/Testing/addAttendance",
                    //data: frm.serialize(),
