...
<?php if (session()->get('role') == 1) { ?>
	<a href="<?= base_url('admin/PdfBuilder/donarReportNewPdf/') . encryptor('encrypt', $studentid) ?>" class="btn btn-success waves-effect waves-light m-b-5 m-l-5  pull-left" target="_blank" style="display:none;">Donor Report</a>

<?php } ?>
<div class="row" style="padding:3px;background: #f5f5f5;"></div>
<table class="table table-striped table-bordered">
	<thead>
		<tr>
			<th width="5%">S.No </th>
			<th width="10%">Received Date <span class="requires">*</span></th>
			<th>Payment Type <span class="requires">*</span></th>
			<th>Check Number </th>
			<th>Amount <span class="requires">*</span></th>
			<th>Campaign <span class="requires">*</span></th>
			<th>Start Date</th>
			<th>End Date</th>
			<th>GrantID</th>
			<th>SoftCredit</th>
	...
	<tbody class="tbl-body-donation">
		<?php
		//Start Fwd: Sort on donation /payment tab
		$ref = $organizationDonation;
		$ref_count = !empty($ref) ? count($ref) : 0;
		if ($access['add_access']) { ?>
			<tr id="TextBoxDivDP<?php echo $ref_count + 1; ?>">
				...
				<td>
					...
					<input class="form-control datepicker" id="start_date<?php echo $ref_count + 1; ?>" name="start_date[<?php echo $ref_count + 1; ?>]" type="text">
				</td>
				<td>
					<span class="hide"></span>
					<input class="form-control datepicker" id="end_date<?php echo $ref_count + 1; ?>" name="end_date[<?php echo $ref_count + 1; ?>]" type="text">
				</td>


				<td>
					<span class="hide"></span>
					<input type="text" class="form-control" name="GrantID[<?php echo $ref_count + 1; ?>]" id="GrantID<?php echo $ref_count + 1; ?>" maxlength="20">
				</td>
				<td>
					<span class="hide"></span>
					<input type="checkbox" name="SoftCredit[<?php echo $ref_count + 1; ?>]" value="Yes" id="SoftCredit<?php echo $ref_count + 1; ?>">
				</td>
			...
			<tr id="TextBoxDivDPN<?php echo $ref_count + 1; ?>">
				...
				<td colspan="2">
					<span class="hide"></span>
					<textarea class="form-control" name="short_note[<?php echo $ref_count + 1; ?>]" id="short_note<?= $ref_count + 1 ?>"></textarea>
				</td>
				<td colspan="2">
			...
			<?php 	}
		//End Fwd: Sort on donation /payment tab
			...
			?>
				<tr id="TextBoxDivDP<?php echo $ref_count; ?>">
					...
					<td style="text-align:left;">
						...
						<input type="hidden" name="DonorID[<?= $ref_count; ?>]" id="DonorID<?= $ref_count; ?>" value="<?php if (isset($user['org_id'])) {
																															echo $user['org_id'];
																														} ?>">
					...
					<td>
						...
						echo "<i class='fa fa-pencil edit_course' " . $display_sec . " id='edit_button" . $ref_count . "' rel_id=" . $ref_count . " rel_course=" . isset($user['course_id']) . " rel_credit=" . isset($user['credit']) . " style='cursor:pointer;'></i>"; ?>
					...
					<td>
						<span class="show">
							...
							if (isset($user['Campaign'])) {
								$cmp_result = campaignsOrganizationName($user['Campaign']);
								if (is_array($cmp_result) && isset($cmp_result['CampaignName'])) {
									echo $cmp_result['CampaignName'];
								} else {
									echo "N/A"; // Optional fallback
								}
							}
							?>
						</span>
					...
					<td>
						<span class="show"><?php if (isset($user['start_date'])) {
												echo convertDateString($user['start_date']);
											} ?></span>
						<input class="form-control hide datepicker" id="start_date<?php echo $ref_count; ?>" name="start_date[<?php echo $ref_count; ?>]" type="text" value="<?php if (isset($user['start_date'])) {
																																													echo convertDateString($user['start_date']);
																																												} ?>">
					</td>
					<td>
						<span class="show"><?php if (isset($user['end_date'])) {
												echo convertDateString($user['end_date']);
											} ?></span>
						<input class="form-control hide datepicker" id="end_date<?php echo $ref_count; ?>" name="end_date[<?php echo $ref_count; ?>]" type="text" value="<?php if (isset($user['end_date'])) {
																																												echo convertDateString($user['end_date']);
																																											} ?>">
					</td>
					...
					<td>
						<span class="show"><?php if (isset($user['GrantID'])) {
												echo $user['GrantID'];
											} ?></span>
						<input type="text" class="form-control hide" name="GrantID[<?php echo $ref_count; ?>]" value="<?php if (isset($user['GrantID'])) {
																															echo $user['GrantID'];
																														} ?>" id="GrantID<?php echo $ref_count; ?>" maxlength="20">
					</td>
					<td>
						<span class="show">
							<?php if (isset($user['SoftCredit'])) {
								echo $user['SoftCredit'];
							} ?>
						</span>
						<input type="checkbox" <?php if (isset($user['SoftCredit'])) {
													if ($user['SoftCredit'] == 'Yes') {
														echo 'checked';
													}
												} ?> class="hide" value="Yes" name="SoftCredit[<?php echo $ref_count; ?>]" id="SoftCredit<?php echo $ref_count; ?>">
					</td>
					...
					<td style="width:5%;text-align:center; vertical-align:middle;" rowspan="2">
						<?php if (session()->get('role') == '1' || in_array(3, session()->get('profiles'))) { ?>
							<a href="javascript:void(0)" id="edit-donation<?php echo $ref_count; ?>" class=" waves-effect waves-light btn-xs m-b-1 edit-donation show pull-left" data-id="<?= isset($user['DonorID']) ?>" data-row="<?= $ref_count ?>" style="text-align:center;">
								<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>
								<span><strong>Edit</strong></span>
							</a>
						<?php } ?>
						<a href="javascript:void(0)" id="save-donation<?php echo $ref_count; ?>" class="btn btn-success waves-effect waves-light btn-xs m-b-1 save-donation hide pull-left save<?= $ref_count; ?>" data-id="<?= isset($user['DonorID']) ?>" data-row="<?= $ref_count ?>">
							<span class="glyphicon glyphicon-ok" aria-hidden="true"></span>
							<span><strong>Save</strong></span>
						</a>
						...
						<a href="javascript:void(0)" rel_donation_id="<?= $user['Donor_RowID'] ?>" id="delete-donation<?php echo $ref_count; ?>" class="btn btn-danger waves-effect waves-light btn-xs m-b-5 delete-donation hide pull-left delete<?= $ref_count; ?>" data-id="<?= isset($user['DonorID']) ?>" data-row="<?= $ref_count ?>">
							<span class="glyphicon glyphicon-trash" aria-hidden="true"></span>
							<span><strong>Delete</strong></span>
						</a>

						<a href="javascript:void(0)" id="cancel-donation<?php echo $ref_count; ?>" class="btn btn-primary waves-effect waves-light btn-xs m-b-5 cancel-donation hide pull-left" data-id="<?= isset($user['DonorID']) ?>" data-row="<?= $ref_count ?>">
							<span class="glyphicon glyphicon-remove" aria-hidden="true"></span>
							<span><strong>Cancel</strong></span>
				...
				<tr id="TextBoxDivDPN<?php echo $ref_count; ?>">
					<td><strong>Note :</strong></td>
					<td colspan="2">
						<span class="show"><?= $user['short_note'] ?></span>
						<textarea class="form-control hide" name="short_note[<?php echo $ref_count; ?>]" id="short_note<?= $ref_count ?>"><?= $user['short_note'] ?></textarea>
					</td>
					<td colspan="2"><strong>Donation Note :</strong>
						<input type="hidden" name="course[<?php echo $ref_count; ?>]" id="denotion_course<?= $ref_count ?>" value="<?= isset($user['course_id']) ?>">
						<input type="hidden" name="student_credit[<?php echo $ref_count; ?>]" id="student_credit<?= $ref_count ?>" value="<?= isset($user['credit']) ?>">


						<input type="hidden" name="student_credit_note[<?php echo $ref_count; ?>]" id="student_credit_note<?= $ref_count ?>" value="<?= isset($user['credit_note']) ?>">
						<input type="hidden" name="scholar_adjustment[<?php echo $ref_count; ?>]" id="scholar_adjustment<?php echo $ref_count; ?>" value="<?= isset($user['scholor_adjustment']) ?>">
						<input type="hidden" name="scholar_adjustment_note[<?php echo $ref_count; ?>]" id="scholar_adjustment_note<?php echo $ref_count; ?>" value="<?= isset($user['scholor_adjustment_note']) ?>">
					...
					<div class="col-md-6">
						<div class="form-group">
							...
							<select class="form-control paymentclass">
								...
								<?php if (isset($assign_class) && is_array($assign_class)) : ?>
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<div class="form-group">
					...
					<input type="text" style="text-align:left;" readonly class="form-control" value="<?= service('uri')->getSegment(4) ?>" id="emp_id">
				</div>

				<div class="form-group">
					<label>First Name</label>
					<input type="text" style="text-align:left;" readonly class="form-control" id="first_name" value="<?= isset($studentinformation['FirstName']) ?>">
				</div>

				<div class="form-group">
					<label>Last Name</label>
					<input type="text" style="text-align:left;" readonly class="form-control" id="last_name" value="<?= isset($studentinformation['LastName']) ?>">
				</div>

				<div class="form-group">
					<label>Amount</label>

					<input type="text" style="text-align:left;" readonly class="form-control" id="modal_amount"
						value="<?= is_array($get_amount) && isset($get_amount['field_value']) ? $get_amount['field_value'] : '' ?>">
				...
				<div class="form-group">
					...
					<input type="text" style="text-align:left;" readonly class="form-control" id="pay_type" value="<?= isset($pay_type['field_value']) ?>">
				</div>
