...
namespace ThstiServer.Controllers
{
    ...
    public class ProgrammesController : ControllerBase
    {
        ...
        public ProgrammesController(ThstiDbContext context)
        {
            _context = context;
        }
        ...
        {
            var item = await _context.Programmes.FindAsync(id);
        ...
        [Authorize(Roles = "ADMIN,MANAGER,EXECUTIVE")]
        [HttpPut("{id:long}")]
        public async Task<IActionResult> Update(long id, [FromBody] Programme updatedItem)
        {
            var item = await _context.Programmes.FindAsync(id);
            if (item == null) return NotFound();
        ...
        public async Task<IActionResult> Delete(long id)
        {
            ...
            _context.Programmes.Remove(item);
            await _context.SaveChangesAsync();
            return NoContent();
        }
        ...
        {
            var item = await _context.Programmes.FirstOrDefaultAsync(x => x.Slug == slug);
            if (item == null) return NotFound();
            return Ok(item);
        }
        ...
        public async Task<IActionResult> Reorder([FromBody] ThstiServer.DTOs.GenericReorderRequest req)
        {
            ...
            try
            {
                ...
                {
                    var item = await _context.Programmes.FindAsync(order.Id);
